...
async def test_adapter_basic_operations(cache_adapter):
    ...
    # Set a value
    success = await cache_adapter.set(
        "test_func",
        args=(1, 2),
        kwargs={"a": "b"},
        result="test_result"
    ...
    # Get the value
    hit, result = await cache_adapter.get(
        "test_func",
        args=(1, 2),
        kwargs={"a": "b"}
    )
    ...
    # Invalidate
    success = await cache_adapter.invalidate(
        "test_func",
        args=(1, 2),
        kwargs={"a": "b"}
    )
...
async def test_custom_key_functions():
    ...
    try:
        ...
        key_fn = compose_key_functions(
            extract_key_component("kwargs.user_id"),
            extract_key_component("metadata.version")
        )
